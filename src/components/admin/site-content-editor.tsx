...
function collectContentImageUrls(value: unknown, urls: Set<string>) {
  ...
  Object.entries(value as Record<string, unknown>).forEach(([key, child]) => {
    if (key === "image_url" && typeof child === "string" && canOptimizeImageUrl(child))
      urls.add(child);
    else collectContentImageUrls(child, urls);
  });
}

function replaceContentImageUrls(
  value: unknown,
  results: Map<string, ImageOptimizationResult>,
): unknown {
    ...
    <details className="group border border-border bg-background">
      <summary className="cursor-pointer list-none border-b border-transparent px-5 py-4 hover:bg-secondary/40 group-open:border-border">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] tracking-[0.2em] text-muted-foreground group-open:hidden">
            EDIT ↓
          </span>
          <span className="hidden text-[10px] tracking-[0.2em] text-muted-foreground group-open:inline">
            CLOSE ↑
          </span>
        </div>
      </summary>
...
export function SiteContentEditor() {
  ...
  async function optimizeAllImages() {
    ...
    if (
      !window.confirm(
        `Optimize ${urls.size} unique images now? Originals will be kept for recovery.`,
      )
    )
      return;
    ...
    try {
      ...
      for (const product of products) {
        ...
        const changed =
          ...
          galleryUrls.some((url, i) => url !== product.gallery_urls[i]);
        if (changed) {
      ...
      for (const key of Object.keys(content) as SiteContentKey[]) {
        ...
        if (JSON.stringify(updated) !== JSON.stringify(current))
          await save({ data: { key, data: updated } });
      }
  ...
  return (
    <div className="mt-8 grid gap-3">
      <p className="text-sm text-muted-foreground">
        Edit every section of the storefront — header, hero, trust bar, story, section titles, CTA
        strip and footer. Changes go live immediately.
      </p>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">
              Compress product, gallery and site-content images to lightweight WebP files. Already
              optimized images are skipped, and original files are retained.
            </p>
          </div>
          <button
            type="button"
        ...
        {optimizationProgress && (
          <div className="mt-4" aria-live="polite">
            <div className="mb-1.5 flex justify-between text-[11px] text-muted-foreground">
              <span>
                {optimizationProgress.done} of {optimizationProgress.total} processed
              </span>
              <span>
                {optimizationProgress.optimized} optimized · {optimizationProgress.skipped} skipped
                · {optimizationProgress.failed} failed
              </span>
            </div>
            <div className="h-1.5 overflow-hidden bg-border">
              <div
                ...
                style={{
                  width: `${optimizationProgress.total ? (optimizationProgress.done / optimizationProgress.total) * 100 : 0}%`,
                }}
              />
            </div>
            {optimizationProgress.done === optimizationProgress.total && (
              <p className="mt-2 text-xs text-muted-foreground">
                Space and transfer saved: {formatSavedBytes(optimizationProgress.bytesSaved)}
              </p>
            )}
          </div>
        )}
                ...
                <button
                  ...
                  onClick={() =>
                    set((p) => ({
                      ...
                      announcements: [
                        ...p.announcements,
                        { icon: "Sparkles" as IconName, text: "" },
                      ],
                    }))
                  }
                  ...
                  <div key={i} className="grid grid-cols-[140px_1fr_auto] gap-2">
                    <IconPicker
                      ...
                      onChange={(v) =>
                        set((p) => ({
                          ...p,
                          announcements: p.announcements.map((x, ix) =>
                            ix === i ? { ...x, icon: v } : x,
                          ),
                        }))
                      }
                    ...
                    <button
                      ...
                      onClick={() =>
                        set((p) => ({
                          ...p,
                          announcements: p.announcements.filter((_, ix) => ix !== i),
                        }))
                      }
          ...
          <>
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() =>
                    set((p) => ({
                      ...p,
                      items: [...p.items, { label: "", to: "/products", hash: "" }],
                    }))
                  }
                  className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                >
                ...
                {s.items.map((it, i) => (
                  <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        set((p) => ({ ...p, items: p.items.filter((_, ix) => ix !== i) }))
                      }
                      className="rounded p-2 text-muted-foreground hover:text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
              ...
              <p className="mt-2 text-[11px] text-muted-foreground">
                Tip: use path <code>/products</code> and an anchor like <code>rings</code> to jump
                straight to a category on the shop page.
              </p>
            </div>
          </>
        )}
          ...
          <>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Title line 1">
                <input
                  className={inputCls}
                  value={s.title_line_1}
                  onChange={(e) => set({ title_line_1: e.target.value })}
                />
              </Field>
              <Field label="Title line 2">
                <input
                  className={inputCls}
                  value={s.title_line_2}
                  onChange={(e) => set({ title_line_2: e.target.value })}
                />
              </Field>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Primary button label">
                <input
                  className={inputCls}
                  value={s.primary_cta_label}
                  onChange={(e) => set({ primary_cta_label: e.target.value })}
                />
              </Field>
              <Field label="Primary button anchor (e.g. rings)">
                <input
                  className={inputCls}
                  value={s.primary_cta_hash}
                  onChange={(e) => set({ primary_cta_hash: e.target.value })}
                />
              </Field>
              ...
              <Field label="Secondary button anchor">
                <input
                  className={inputCls}
                  value={s.secondary_cta_hash}
                  onChange={(e) => set({ secondary_cta_hash: e.target.value })}
                />
              </Field>
            </div>
            <Field label="Custom card title">
              <input
                className={inputCls}
                value={s.custom_card_title}
                onChange={(e) => set({ custom_card_title: e.target.value })}
              />
            </Field>
            <Field label="Custom card body">
              <textarea
                rows={2}
                className={inputCls}
                value={s.custom_card_body}
                onChange={(e) => set({ custom_card_body: e.target.value })}
              />
            </Field>
          </>
        )}
          ...
          <div className="grid gap-3">
              ...
              <div key={i} className="grid grid-cols-[140px_1fr_1fr_auto] gap-2">
                <IconPicker
                  ...
                  onChange={(v) =>
                    set((p) => ({
                      ...p,
                      items: p.items.map((x, ix) => (ix === i ? { ...x, icon: v } : x)),
                    }))
                  }
                />
                <input
                ...
                <button
                  type="button"
                  onClick={() => set((p) => ({ ...p, items: p.items.filter((_, ix) => ix !== i) }))}
                  className="rounded p-2 text-muted-foreground hover:text-destructive"
                >
                  <Trash2 className="h-4 w-4" />
                </button>
            ...
            <button
              ...
              onClick={() =>
                set((p) => ({
                  ...p,
                  items: [...p.items, { icon: "Sparkles" as IconName, title: "", body: "" }],
                }))
              }
              className="inline-flex items-center gap-1 justify-self-start text-xs text-gold hover:underline"
            >
      ...
      <SectionCard
        ...
        render={(s, set) => (
          <>
            ...
            <ImageUploadField
              label="Image (leave blank for default)"
              value={s.image_url}
          ...
          <div className="grid gap-3">
                ...
                <input
                  ...
                  onChange={(e) =>
                    set((p) => ({
                      ...p,
                      items: p.items.map((x, ix) =>
                        ix === i ? { ...x, subtitle: e.target.value } : x,
                      ),
                    }))
                  }
                />
            ...
            <button
              ...
              onClick={() =>
                set((p) => ({
                  ...p,
                  items: [...p.items, { icon: "Sparkles" as IconName, title: "", subtitle: "" }],
                }))
              }
              className="inline-flex items-center gap-1 justify-self-start text-xs text-gold hover:underline"
            >
          ...
          <div className="grid gap-5">
              ...
              <div key={i} className="grid gap-3 border border-border p-4">
                <div className="flex items-center justify-between">
                  ...
                  {s.slides.length > 1 && (
                    <button
                      type="button"
                      onClick={() =>
                        set((p) => ({ ...p, slides: p.slides.filter((_, ix) => ix !== i) }))
                      }
                ...
                <ImageUploadField
                  ...
                  onChange={(url) =>
                    set((p) => ({
                      ...p,
                      slides: p.slides.map((x, ix) => (ix === i ? { ...x, image_url: url } : x)),
                    }))
                  }
                />
                <Field label="Destination link (e.g. /products, /custom-jewellery, or https://...) ">
                  <input
                    className={inputCls}
                    ...
                    onChange={(e) =>
                      set((p) => ({
                        ...p,
                        slides: p.slides.map((x, ix) =>
                          ix === i ? { ...x, link: e.target.value } : x,
                        ),
                      }))
                    }
                  />
                </Field>
                <Field label="Image description (for accessibility)">
                  <input
                    className={inputCls}
                    placeholder="New arrivals in 925 silver"
                    value={slide.alt}
                    onChange={(e) =>
                      set((p) => ({
                        ...p,
                        slides: p.slides.map((x, ix) =>
                          ix === i ? { ...x, alt: e.target.value } : x,
                        ),
                      }))
                    }
                  />
            ...
            <button
              ...
              onClick={() =>
                set((p) => ({
                  ...p,
                  slides: [...p.slides, { image_url: "", link: "/products", alt: "" }],
                }))
              }
              className="inline-flex items-center gap-1 justify-self-start text-xs text-gold hover:underline"
            >
          ...
          <>
            <Field label="Brand blurb">
              <textarea
                ...
                className={inputCls}
                value={s.brand_blurb}
                onChange={(e) => set({ brand_blurb: e.target.value })}
              />
            </Field>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button
                    ...
                    onClick={() =>
                      set((p) => ({
                        ...p,
                        shop_links: [...p.shop_links, { label: "", to: "/products" }],
                      }))
                    }
                    className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                  >
                    ...
                    <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          set((p) => ({
                            ...p,
                            shop_links: p.shop_links.map((x, ix) =>
                              ix === i ? { ...x, label: e.target.value } : x,
                            ),
                          }))
                        }
                      ...
                      <button
                        ...
                        onClick={() =>
                          set((p) => ({
                            ...p,
                            shop_links: p.shop_links.filter((_, ix) => ix !== i),
                          }))
                        }
                        className="rounded p-2 text-muted-foreground hover:text-destructive"
                      >
                ...
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button
                    ...
                    onClick={() =>
                      set((p) => ({
                        ...p,
                        help_links: [...p.help_links, { label: "", to: "/products" }],
                      }))
                    }
                    className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                  >
                    ...
                    <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          set((p) => ({
                            ...p,
                            help_links: p.help_links.map((x, ix) =>
                              ix === i ? { ...x, label: e.target.value } : x,
                            ),
                          }))
                        }
                      ...
                      <button
                        ...
                        onClick={() =>
                          set((p) => ({
                            ...p,
                            help_links: p.help_links.filter((_, ix) => ix !== i),
                          }))
                        }
                        className="rounded p-2 text-muted-foreground hover:text-destructive"
                      >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Newsletter body">
                <input
                  className={inputCls}
                  value={s.newsletter_body}
                  onChange={(e) => set({ newsletter_body: e.target.value })}
                />
              </Field>
            </div>
            <Field label="Copyright (year auto-inserted after ©)">
              <input
                className={inputCls}
                value={s.copyright}
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() =>
                    set((p) => ({ ...p, items: [...p.items, { url: "", caption: "" }] }))
                  }
                  className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        set((p) => ({
                          ...p,
                          items: p.items.map((x, ix) =>
                            ix === i ? { ...x, caption: e.target.value } : x,
                          ),
                        }))
                      }
                    />
              ...
              <p className="mt-2 text-[11px] text-muted-foreground">
                Paste the full URL of a public Reel, post or video (e.g.{" "}
                <code>https://www.instagram.com/reel/ABC123/</code>). Private accounts won't render.
              </p>
            </div>
          </>
        )}
      ...
      <SectionCard
        ...
        render={(s, set) => (
          <>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Public handle (e.g. @yomora)">
                <input
                  className={inputCls}
                  value={s.handle}
              ...
              <Field label="Follow button label">
                <input
                  className={inputCls}
                  value={s.cta_label}
            ...
            <div className="flex flex-wrap gap-4 text-xs">
              <label className="inline-flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={s.show_in_header}
                  onChange={(e) => set({ show_in_header: e.target.checked })}
                />
                ...
                <button
                  ...
                  onClick={() =>
                    set((p) => ({
                      ...
                      items: [
                        ...p.items,
                        { platform: "Instagram" as SocialPlatform, url: "", label: "" },
                      ],
                    }))
                  }
                  className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        set((p) => ({
                          ...p,
                          items: p.items.map((x, ix) =>
                            ix === i ? { ...x, platform: e.target.value as SocialPlatform } : x,
                          ),
                        }))
                      }
                    >
      ...
      <SectionCard
        ...
        render={(s, set) => (
          <>
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() =>
                    set((p) => ({ ...p, stats: [...p.stats, { value: "", label: "" }] }))
                  }
                  className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                >
                  <Plus className="h-3 w-3" /> Add
                  ...
                  <div key={i} className="grid grid-cols-[1fr_2fr_auto] gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        set((p) => ({
                          ...p,
                          stats: p.stats.map((x, ix) =>
                            ix === i ? { ...x, value: e.target.value } : x,
                          ),
                        }))
                      }
                    />
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        set((p) => ({ ...p, stats: p.stats.filter((_, ix) => ix !== i) }))
                      }
                      className="rounded p-2 text-muted-foreground hover:text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
            ...
            <Field label="Store section title">
              <input
                className={inputCls}
                value={s.store_title}
                onChange={(e) => set({ store_title: e.target.value })}
              />
            </Field>
          </>
        )}
      ...
      <SectionCard
        ...
        render={(s, set) => (
          <>
            ...
            <Field label="Steps eyebrow">
              <input
                className={inputCls}
                value={s.steps_eyebrow}
                onChange={(e) => set({ steps_eyebrow: e.target.value })}
              />
            </Field>
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() =>
                    set((p) => ({
                      ...
                      steps: [
                        ...p.steps,
                        { icon: "Sparkles" as IconName, title: "", description: "" },
                      ],
                    }))
                  }
                  className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                  ...
                  <div key={i} className="grid grid-cols-[140px_1fr_2fr_auto] gap-2">
                    <IconPicker
                      ...
                      onChange={(v) =>
                        set((p) => ({
                          ...p,
                          steps: p.steps.map((x, ix) => (ix === i ? { ...x, icon: v } : x)),
                        }))
                      }
                    />
                    <input
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        set((p) => ({
                          ...p,
                          steps: p.steps.map((x, ix) =>
                            ix === i ? { ...x, description: e.target.value } : x,
                          ),
                        }))
                      }
                    />
                    <button
                      type="button"
                      onClick={() =>
                        set((p) => ({ ...p, steps: p.steps.filter((_, ix) => ix !== i) }))
                      }
                      className="rounded p-2 text-muted-foreground hover:text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Button label">
                <input
                  className={inputCls}
                  value={s.form_button_label}
                  onChange={(e) => set({ form_button_label: e.target.value })}
                />
              </Field>
            </div>
          ...
          <>
            ...
            <Field label="Phone lines (one per line)">
              <textarea
                rows={3}
                className={inputCls}
                value={s.phone_lines.join("\n")}
                onChange={(e) =>
                  set({
                    phone_lines: e.target.value
                      .split("\n")
                      .map((x) => x.trim())
                      .filter(Boolean),
                  })
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() =>
                    set((p) => ({ ...p, items: [...p.items, { question: "", answer: "" }] }))
                  }
                  className="inline-flex items-center gap-1 text-xs text-gold hover:underline"
                >
                  <Plus className="h-3 w-3" /> Add
                ...
                {s.items.map((it, i) => (
                  <div key={i} className="grid gap-2 border border-border p-3">
                    <input
                      ...
                      onChange={(e) =>
                        set((p) => ({
                          ...p,
                          items: p.items.map((x, ix) =>
                            ix === i ? { ...x, question: e.target.value } : x,
                          ),
                        }))
                      }
                    />
                    <textarea
                      ...
                      onChange={(e) =>
                        set((p) => ({
                          ...p,
                          items: p.items.map((x, ix) =>
                            ix === i ? { ...x, answer: e.target.value } : x,
                          ),
                        }))
                      }
                    />
                    <button
                      ...
                      className="justify-self-end text-xs text-muted-foreground hover:text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <Field label="Aside title">
                <input
                  className={inputCls}
                  value={s.aside_title}
                  onChange={(e) => set({ aside_title: e.target.value })}
                />
              </Field>
              <Field label="Aside body">
                <input
                  className={inputCls}
                  value={s.aside_body}
                  onChange={(e) => set({ aside_body: e.target.value })}
                />
              </Field>
              <Field label="Aside button">
                <input
                  className={inputCls}
                  value={s.aside_button_label}
                  onChange={(e) => set({ aside_button_label: e.target.value })}
                />
              </Field>
            </div>
          </>
      ...
      <SectionCard
        ...
        render={(s, set) => (
          <>
            ...
            <Field label="Tagline fallback">
              <input
                className={inputCls}
                value={s.tagline_fallback}
                onChange={(e) => set({ tagline_fallback: e.target.value })}
              />
            </Field>
            <Field label="Unlock box title">
              <input
                className={inputCls}
                value={s.unlock_title}
                onChange={(e) => set({ unlock_title: e.target.value })}
              />
            </Field>
            <div className="grid gap-4 sm:grid-cols-3">
              <Field label="Pay label">
                <input
                  className={inputCls}
                  value={s.pay_label}
                  onChange={(e) => set({ pay_label: e.target.value })}
                />
              </Field>
              ...
              <Field label="Or label">
                <input
                  className={inputCls}
                  value={s.or_label}
                  onChange={(e) => set({ or_label: e.target.value })}
                />
              </Field>
              <Field label="Shop amount label">
                <input
                  className={inputCls}
                  value={s.shop_amount_label}
                  onChange={(e) => set({ shop_amount_label: e.target.value })}
                />
              </Field>
              <Field label="Shop note">
                <input
                  className={inputCls}
                  value={s.shop_note}
                  onChange={(e) => set({ shop_note: e.target.value })}
                />
              </Field>
              <Field label="Validity note">
                <input
                  className={inputCls}
                  value={s.validity_note}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Card subtitle">
                <input
                  className={inputCls}
                  value={s.card_subtitle}
                  onChange={(e) => set({ card_subtitle: e.target.value })}
                />
              </Field>
              <Field label="Card line 1">
                <input
                  className={inputCls}
                  value={s.card_line_1}
                  onChange={(e) => set({ card_line_1: e.target.value })}
                />
              </Field>
              <Field label="Card line 2">
                <input
                  className={inputCls}
                  value={s.card_line_2}
                  onChange={(e) => set({ card_line_2: e.target.value })}
                />
              </Field>
              <Field label="Card line 3">
                <input
                  className={inputCls}
                  value={s.card_line_3}
                  onChange={(e) => set({ card_line_3: e.target.value })}
                />
              </Field>
            </div>
            <Field label="Privileges section title">
              <input
                className={inputCls}
                value={s.privileges_title}
                onChange={(e) => set({ privileges_title: e.target.value })}
              />
            </Field>
            <Field label="Privileges footer">
              <input
                className={inputCls}
                value={s.privileges_footer}
                onChange={(e) => set({ privileges_footer: e.target.value })}
              />
            </Field>
            <div>
              <div className="mb-2 flex items-center justify-between">
                  ...
                  <div key={i} className="grid grid-cols-[140px_1fr_2fr_auto] gap-2">
                    <IconPicker
                      ...
                      onChange={(v) =>
                        set((p) => ({
                          ...p,
                          privileges: p.privileges.map((x, ix) =>
                            ix === i ? { ...x, icon: v } : x,
                          ),
                        }))
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        set((p) => ({ ...p, privileges: p.privileges.filter((_, ix) => ix !== i) }))
                      }
                      className="rounded p-2 text-muted-foreground hover:text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Order ID label">
                <input
                  className={inputCls}
                  value={s.order_id_label}
                  onChange={(e) => set({ order_id_label: e.target.value })}
                />
              </Field>
              <Field label="Email label">
                <input
                  className={inputCls}
                  value={s.email_label}
                  onChange={(e) => set({ email_label: e.target.value })}
                />
              </Field>
              <Field label="Button label">
                <input
            ...
            <Field label="Empty state message">
              <input
                className={inputCls}
                value={s.empty_message}
                onChange={(e) => set({ empty_message: e.target.value })}
              />
            </Field>
          </>
        )}

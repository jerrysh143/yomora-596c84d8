...
function InvoicePage() {
  ...
  const tax = details.tax_rate ? Math.round(Math.max(0, subtotal - discount) * details.tax_rate / 100) : 0;
  ...
  return <div className="min-h-screen bg-[#f7f2eb] text-[#1d1815]">
    ...
    <main className="invoice-shell mx-auto max-w-[980px] px-4 py-10 md:py-14">
      ...
      <section className="border border-[#d8cfc3] bg-white p-7 shadow-sm md:p-12">
        ...
        <div className="mt-7 ml-auto max-w-sm text-sm"><div className="flex justify-between py-2 text-[#665b50]"><span>Subtotal</span><span>{formatINR(subtotal)}</span></div>{discount > 0 && <div className="flex justify-between py-2 text-[#665b50]"><span>{order.coupon_code ? `Coupon (${order.coupon_code})` : "Discount"}</span><span>− {formatINR(discount)}</span></div>}{details.tax_rate ? <div className="flex justify-between py-2 text-[#665b50]"><span>Tax ({details.tax_rate}%)</span><span>{formatINR(tax)}</span></div> : null}<div className="mt-2 flex justify-between border-t-2 border-[#1d1815] pt-3 font-display text-xl"><span>TOTAL</span><span>{formatINR(order.total)}</span></div></div>

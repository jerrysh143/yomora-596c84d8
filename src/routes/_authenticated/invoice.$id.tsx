  ...
  return <div className="min-h-screen bg-[#f7f2eb] text-[#1d1815]">
    ...
    <main className="invoice-shell mx-auto max-w-[980px] px-4 py-10 md:py-14">
      ...
      <section className="border border-[#d8cfc3] bg-white p-7 shadow-sm md:p-12">
        ...
        <div className="mt-7 ml-auto max-w-sm text-sm"><div className="flex justify-between py-2 text-[#665b50]"><span>Product Amount (before GST)</span><span>{formatTaxINR(tax.productAmount)}</span></div>{discount > 0 && <div className="flex justify-between py-2 text-[#665b50]"><span>{order.coupon_code ? `Discount (${order.coupon_code})` : "Discount"}</span><span>− {formatTaxINR(tax.discountAmount)}</span></div>}<div className="flex justify-between border-y border-[#ded6cc] py-3 font-semibold"><span>Taxable Amount</span><span>{formatTaxINR(tax.taxableAmount)}</span></div><div className="flex justify-between py-2 text-[#665b50]"><span>GST @ {taxRate}%</span><span>{formatTaxINR(tax.gstAmount)}</span></div><div className="mt-2 flex justify-between border-t-2 border-[#1d1815] pt-3 font-display text-xl"><span>INVOICE TOTAL</span><span>{formatTaxINR(tax.invoiceTotal)}</span></div></div>

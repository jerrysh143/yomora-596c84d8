...
export const Route = createFileRoute("/account")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Manage your YOMORA orders, addresses, wishlist and membership.",
      },
...
function AccountPage() {
  ...
  const [tab, setTab] = useState<
    "dashboard" | "orders" | "addresses" | "wishlist" | "reviews" | "details"
  >("dashboard");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const savedAddresses = Array.isArray(metadata.addresses)
        ? metadata.addresses.filter(
            (address: unknown): address is Address =>
              !!address &&
              typeof address === "object" &&
              typeof (address as Address).id === "string" &&
              typeof (address as Address).value === "string",
          )
        : [];
  ...
  const addAddress = async () => {
    const fields = Object.fromEntries(
      Object.entries(newAddress).map(([key, value]) => [key, value.trim()]),
    ) as AddressDraft;
    if (
      !fields.full_name ||
      !fields.house_number ||
  ...
  const removeAddress = (id: string) =>
    saveProfile({
      ...profile,
      addresses: profile.addresses.filter((address) => address.id !== id),
    });
        ...
        <div className="grid gap-8 md:grid-cols-[260px_1fr]">
          <aside className="border border-border p-5 h-max">
            <div className="flex items-center gap-3">
              <div className="grid h-12 w-12 place-items-center rounded-full bg-gold text-onyx font-display text-xl">
                {(profile.full_name?.[0] || email?.[0] || "Y").toUpperCase()}
              </div>
            ...
            <ul className="mt-6 space-y-1 text-sm">
              {menu.map(([key, label, Icon]) => (
                <li key={key}>
                  {key === "membership" ? (
                    ...
                    <button
                      onClick={() => setTab(key)}
                      className={`flex w-full items-center gap-3 rounded px-3 py-2 text-left ${tab === key ? "bg-gold text-onyx" : "hover:bg-secondary/60"}`}
                    >
                      <Icon className="h-4 w-4" /> {label}
                    </button>
                  )}
              ...
              <li>
                <button
                  onClick={signOut}
                  className="flex w-full items-center gap-3 rounded px-3 py-2 text-left text-destructive hover:bg-destructive/10"
                >
...
function Dashboard({
  ...
  reviewCount: number;
  memberLabel: string;
  setTab: (tab: "orders" | "wishlist") => void;
}) {
  return (
    <>
      ...
      <div className="mt-10 border border-border">
        ...
        {orders.length ? (
          ...
          <div className="p-6 text-sm text-muted-foreground">
            No orders yet. Your placed orders will appear here automatically.
          </div>
        )}
      </div>
      <p className="mt-5 text-xs text-muted-foreground">
        {memberLabel} membership details are available from the left menu.
      </p>
    </>
  );
}
...
function Stat({ value, label }: { value: string | number; label: string }) {
  return (
    <div className="border border-border p-6 text-center">
      ...
      <div className="mt-1 text-[11px] tracking-[0.24em] text-muted-foreground">
        {label.toUpperCase()}
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="mt-6 border border-border">
        {loading ? (
          ...
          <div className="p-6 text-sm text-muted-foreground">No orders found for this account.</div>
        )}
      </div>
    </>
  );
}
function MyReviews({ reviews, loading }: { reviews: ProductReview[]; loading: boolean }) {
  return (
    <>
      ...
      <div className="mt-6 grid gap-4">
        {loading ? (
          ...
          reviews.map((review) => (
            <article key={review.id} className="border border-border p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <Link
                  to="/products/$category"
                  params={{ category: review.product_id }}
                  hash="reviews"
                  className="text-xs font-semibold tracking-[0.16em] text-gold hover:text-foreground"
                >
                  VIEW PRODUCT
                </Link>
                <span className="flex text-gold">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      className={`h-4 w-4 ${star <= review.rating ? "fill-current" : "opacity-25"}`}
                    />
                  ))}
                </span>
              </div>
                  ...
                  {review.media_urls.map((url) =>
                    /\.(mp4|webm|mov)(?:\?|$)/i.test(url) ? (
                      <video
                        ...
                        controls
                        preload="metadata"
                        className="h-24 w-24 shrink-0 bg-onyx object-cover"
                      />
                    ) : (
                      <img
                        key={url}
              ...
              <time className="mt-3 block text-[10px] tracking-[0.14em] text-muted-foreground">
          ...
          <p className="border border-border p-6 text-sm text-muted-foreground">
            You have not submitted a review yet. Review buttons appear under delivered orders.
          </p>
        )}
      </div>
    </>
...
function Addresses({
  ...
  setNewAddress: React.Dispatch<React.SetStateAction<AddressDraft>>;
  addAddress: () => void;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch(`https://api.postalpincode.in/pincode/${newAddress.pincode}`, {
      ...
      <div className="mt-6 grid gap-3">
        {addresses.length ? (
          addresses.map((address) => (
            ...
            >
              <div>
                <p className="text-xs tracking-[0.18em] text-gold">{address.label.toUpperCase()}</p>
                <p className="mt-2 whitespace-pre-wrap text-sm">{address.value}</p>
              </div>
              <button
                aria-label={`Delete ${address.label}`}
                onClick={() => removeAddress(address.id)}
                className="grid h-9 w-9 shrink-0 place-items-center text-muted-foreground hover:text-destructive"
              >
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {items.length ? (
          items.map((item) => (
            <article
              key={item.id}
              className="flex items-start gap-4 border border-border p-3 transition-colors hover:border-gold"
            >
              <Link to="/products/$category" params={{ category: item.id }} className="shrink-0">
                <img
                  src={item.image}
                  alt={item.name}
                  width={80}
              ...
              <button
                ...
                className="grid h-9 w-9 shrink-0 place-items-center text-muted-foreground transition-colors hover:text-destructive"
              >
                <Trash2 className="h-4 w-4" />
              </button>
            </article>
...
function Details({
  ...
  return (
    <>
      <h1 className="font-display text-4xl">Account Details</h1>
      <div className="mt-6 grid max-w-xl gap-4 border border-border p-6">
        <label className="grid gap-2 text-xs tracking-[0.18em] text-muted-foreground">
          FULL NAME
          <input
            value={profile.full_name}
            onChange={(event) => setProfile({ ...profile, full_name: event.target.value })}
            className="border border-border bg-background px-3 py-2.5 text-sm text-foreground outline-none focus:border-gold"
          />
        </label>
        <label className="grid gap-2 text-xs tracking-[0.18em] text-muted-foreground">
          EMAIL
          <input
            value={email}
            disabled
            className="border border-border bg-secondary/40 px-3 py-2.5 text-sm text-muted-foreground"
          />
        </label>
        <label className="grid gap-2 text-xs tracking-[0.18em] text-muted-foreground">
          PHONE NUMBER
        ...
        <label className="flex items-start gap-3 border border-border p-4 text-xs text-muted-foreground">
          <input
            type="checkbox"
            checked={profile.marketing_opt_in}
            onChange={(event) => setProfile({ ...profile, marketing_opt_in: event.target.checked })}
            className="mt-0.5"
          />
          <span>
            <b className="block text-foreground">
              Future updates: {profile.marketing_opt_in ? "Active" : "Inactive"}
            </b>
            <span className="mt-1 block">
              Receive YOMORA product launches, offers and membership updates by email or phone.
            </span>
          </span>
        </label>
        <button

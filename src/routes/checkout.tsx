...
function checkoutErrorMessage(error: unknown, fallback: string) {
  ...
  if (
    /Missing Supabase environment variable|SUPABASE_SERVICE_ROLE_KEY|Connect Supabase in Lovable Cloud/i.test(
      message,
    )
  ) {
...
function normaliseSavedAddress(address: CheckoutAddress, fallbackName: string) {
  ...
  return {
    ...
    address_line:
      address.address_line?.trim() || address.value.replace(/(?:,?\s*)\b\d{6}\b\s*$/, "").trim(),
...
function CheckoutPage() {
  ...
  const [pincodeStatus, setPincodeStatus] = useState<"idle" | "loading" | "success" | "error">(
    "idle",
  );
  ...
  useEffect(() => {
    ...
    const applySession = (
      ...
      const addresses = (Array.isArray(metadata.addresses) ? metadata.addresses : []).filter(
        (address: unknown): address is CheckoutAddress =>
          !!address &&
          typeof address === "object" &&
          typeof (address as CheckoutAddress).id === "string" &&
          typeof (address as CheckoutAddress).value === "string",
      );
      const profileName =
  ...
  if (!authReady) {
    return (
      <div className="min-h-screen bg-background">
        <SiteHeader />
        <div className="container-x mx-auto max-w-[1400px] py-24 text-sm text-muted-foreground">
          Checking your YOMORA account…
        </div>
        <SiteFooter />
      </div>
    );
        ...
        {orderId && pay !== "cod" ? (
          <div className="mt-8 grid max-w-5xl gap-8 lg:grid-cols-[360px_1fr]">
            ...
            <div className="border border-border bg-secondary/20 p-6 sm:p-8">
              <p className="text-[11px] font-semibold tracking-[0.24em] text-gold">
                PAYMENT VERIFICATION
              </p>
              <h2 className="mt-2 font-display text-3xl">
                Pay {formatINR(paymentTotal)} using any UPI app
              </h2>
              <p className="mt-3 text-sm leading-6 text-muted-foreground">
                Scan the PhonePe Business QR, pay the exact amount, then upload the
                successful-payment screenshot and enter its UTR or transaction ID.
              </p>
              <div className="mt-5 border border-gold/40 bg-gold/10 p-4">
                ...
                <code className="mt-1 block text-lg font-semibold tracking-[0.12em] text-gold">
                  {paymentCode}
                </code>
              ...
              {proofSubmitted ? (
                <div className="mt-6 flex items-start gap-3 border border-gold bg-onyx p-4 text-cream">
                  <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-gold" />
                  <div>
                    <p className="font-semibold">Payment sent for verification</p>
                    <p className="mt-1 text-xs leading-5 text-white/65">
                      You will receive a YOMORA notification after admin verifies the transaction
                ...
                >
                  ...
                  <label className="block cursor-pointer border border-dashed border-gold/60 bg-background p-4 text-center">
                    ...
                    <span className="mt-2 block text-xs font-semibold tracking-[0.12em]">
                  ...
                  <button
                    disabled={proofSubmitting || !paymentProof || transactionId.length < 8}
                    className="bg-gold px-5 py-3 text-[11px] font-semibold tracking-[0.2em] text-onyx disabled:opacity-40"
                  >
                    {proofSubmitting ? "SUBMITTING…" : "SUBMIT PAYMENT FOR VERIFICATION"}
                  </button>
                </form>
              )}
            </div>
          ...
          <div className="mt-8 max-w-2xl border border-gold bg-gold/10 p-8">
            ...
            <Link
              to="/products"
              className="mt-6 inline-block bg-gold px-5 py-3 text-[11px] font-semibold tracking-[0.2em] text-onyx"
            >
              CONTINUE SHOPPING
            </Link>
          </div>
            ...
            onSubmit={async (e) => {
              ...
              try {
                ...
                if (addressMode === "new" && saveNewAddress) {
                  ...
                  const { error: saveAddressError } = await supabase.auth.updateUser({
                    data: { addresses: [...savedAddresses, savedAddress] },
                  });
                  if (saveAddressError) {
                    console.error("Unable to save checkout address", saveAddressError);
                    toast.warning(
                      "Order placed, but the address could not be saved to My Addresses",
                    );
                  } else {
          ...
          >
            <div className="space-y-8">
              <fieldset className="border border-border p-6">
                <legend className="px-2 text-xs font-semibold tracking-[0.24em] text-gold">
              ...
              <fieldset className="border border-border p-6">
                ...
                {addressMode === "saved" && savedAddresses.length > 0 && (
                  <div className="mb-5 grid gap-2">
                    {savedAddresses.map((address) => (
                      <label
                        key={address.id}
                        className={`flex cursor-pointer items-start gap-3 border p-3 text-sm ${selectedAddressId === address.id ? "border-gold bg-gold/5" : "border-border"}`}
                      >
                        <input
                          type="radio"
                          name="saved_address"
                        ...
                        />
                        <span>
                          <b className="block text-[10px] tracking-[0.16em] text-gold">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    />
                    <p
                      className={`mt-1 text-[10px] ${pincodeStatus === "error" ? "text-destructive" : "text-muted-foreground"}`}
                      aria-live="polite"
                    >
                ...
                {addressMode === "new" && (
                  <label className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />{" "}
                    Save this address in My Addresses
                  </label>
                )}
              </fieldset>
              <fieldset className="border border-border p-6">
                ...
                <div className="space-y-3">
                  <label
                    className={`block cursor-pointer border p-4 transition-colors ${pay === "upi" ? "border-gold bg-gold/10" : "border-border hover:border-gold/60"}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <span className="inline-flex items-center gap-1 text-[9px] font-semibold tracking-[0.16em] text-gold">
                            <ShieldCheck className="h-3.5 w-3.5" /> ADMIN VERIFIED
                          </span>
                        </div>
                        <p className="mt-1 text-xs leading-5 text-muted-foreground">
                          Place the order, scan Devika Jewellers' PhonePe Business QR using any UPI
                          app, and upload the successful transaction screenshot for verification.
                        </p>
                        <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-semibold tracking-[0.1em] text-foreground/75">
                          <span className="inline-flex items-center gap-1 border border-border bg-background px-2 py-1.5">
                            <QrCode className="h-3.5 w-3.5 text-gold" /> PHONEPE QR
                          </span>
                          <span className="inline-flex items-center gap-1 border border-border bg-background px-2 py-1.5">
                            <ShieldCheck className="h-3.5 w-3.5 text-gold" /> UTR + PROOF CHECK
                          </span>
                          <span className="inline-flex items-center gap-1 border border-border bg-background px-2 py-1.5">
                            <LockKeyhole className="h-3.5 w-3.5 text-gold" /> NO UPI PIN STORED
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <label
                    className={`flex cursor-pointer items-center gap-3 border p-4 transition-colors ${pay === "cod" ? "border-gold bg-gold/10" : "border-border hover:border-gold/60"}`}
                  >
                    <input
                      type="radio"
                      name="pay"
                ...
                {pay !== "cod" && (
                  <div className="mt-4 flex items-start gap-2 border-l-2 border-gold bg-secondary/30 px-3 py-2.5 text-[11px] leading-5 text-muted-foreground">
                    <LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-gold" />
                    <span>
                      Your order remains pending until YOMORA Admin matches the UTR, payment
                      screenshot, amount, and unique payment code.
                    </span>
                  </div>
                )}
              </fieldset>
            ...
            <aside className="border border-border p-6 h-max">
              ...
              <dl className="mt-5 space-y-2 text-sm">
                ...
                {discount > 0 && (
                  <Row
                    k={`Discount (${currentCoupon?.code})`}
                    v={`− ${formatTaxINR(tax.discountAmount)}`}
                  />
                )}
              ...
              {!membershipLoading && !hasActiveMembership && (
                <div
                  className={`mt-5 border p-4 ${qualifiesForMembership ? "border-gold bg-gold/10" : "border-border bg-secondary/20"}`}
                >
                  <div className="flex gap-3">
                    <Crown
                      className={`mt-0.5 h-5 w-5 shrink-0 ${qualifiesForMembership ? "text-gold" : "text-muted-foreground"}`}
                    />
                    <div>
                      <p className="text-[10px] font-semibold tracking-[0.2em] text-gold">
                        {qualifiesForMembership
                          ? "COMPLIMENTARY MEMBERSHIP INCLUDED"
                          : "ADD BLACK SIGNATURE MEMBERSHIP"}
                      </p>
                      ...
                      {!qualifiesForMembership && (
                        <Link
                          ...
                          className="mt-3 inline-flex border border-gold px-3 py-2 text-[10px] font-semibold tracking-[0.16em] text-gold transition-colors hover:bg-gold hover:text-onyx"
                        >
                          {activeMembershipPlan
                            ? `GET MEMBERSHIP · ${formatINR(activeMembershipPlan.price)}`
                            : "VIEW MEMBERSHIP"}
                        </Link>
                      )}
                    </div>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          {currentCoupon.description ||
                            `You save ${formatINR(currentCoupon.discount)}`}
                        </p>
                      </div>
                      <button
                        type="button"
              ...
              <button
                ...
                className="mt-6 w-full bg-gold py-3 text-[11px] font-semibold tracking-[0.24em] text-onyx disabled:opacity-40"
              >
                {submitting
                  ? "PLACING ORDER…"
                  : pay === "cod"
                    ? "PLACE ORDER"
                    : "CONTINUE TO PHONEPE QR"}
              </button>
              <p className="mt-3 text-center text-[11px] text-muted-foreground">
                {pay === "cod"
                  ? "Your order will be saved securely in YOMORA Admin."
                  : "Pay using any UPI app. Admin verification is required before order acceptance."}
              </p>
            </aside>
          </form>
        )}

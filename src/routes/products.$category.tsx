...
import { formatINR, productImage, productGallery, isProductNew, type Category, type CategoryRow, type Product, cleanProductName, displayCategoryLabel } from "@/lib/products";
import { relatedProductsFor } from "@/lib/related-products";
...
export const Route = createFileRoute("/products/$category")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Products — YOMORA" }] };
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="bg-onyx text-cream">
        <div className="container-x mx-auto max-w-[1400px] py-14">
          ...
          <h1 className="mt-3 font-display text-5xl">Shop {displayCategoryLabel(category.label, category.slug)}{audience ? ` for ${audience === "men" ? "Him" : "Her"}` : ""}</h1>
      ...
      <section className="container-x mx-auto max-w-[1400px] py-10">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-6">
          <div className="flex flex-wrap gap-2">
            {filters.map((f) => {
              ...
              return f.key === "all" ? (
                <Link key={f.key} to="/products" className={className}>{f.label.toUpperCase()}</Link>
              ) : (
                <Link key={f.key} to="/products/$category" params={{ category: f.key }} className={className}>{f.label.toUpperCase()}</Link>
              );
            })}
          </div>
          ...
          {filteredItems.map((p) => (
            <Link key={p.id} to="/products/$category" params={{ category: p.id }} className="group grid grid-cols-[42%_1fr] items-center gap-2 border border-border/70 bg-secondary/20 p-2 min-[400px]:block min-[400px]:border-0 min-[400px]:bg-transparent min-[400px]:p-0">
              <div className="relative overflow-hidden bg-secondary/40">
                ...
                {p.sold_out && (<span className="absolute inset-x-0 bottom-0 bg-onyx/85 py-2 text-center text-[10px] font-bold tracking-[0.24em] text-cream">SOLD OUT</span>)}
                ...
                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); wishlist.toggle({ id: p.id, name: cleanProductName(p.name), price: p.price, image: productImage(p), category: p.category }); }} aria-label={wishSet.has(p.id) ? "Remove from wishlist" : "Add to wishlist"} className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-background/90 text-onyx hover:bg-gold">
        ...
        {filteredItems.length === 0 && (<p className="py-16 text-center text-sm text-muted-foreground">No pieces in this category yet.</p>)}
      </section>
      <SiteFooter />
    </div>
  );
...
function ProductPage({ product, products }: { product: Product; products: Product[] }) {
  const relatedItems = relatedProductsFor(product, products);
  const nav = useNavigate();
  const img = productImage(product);
  const gallery = productGallery(product);
  const [activeImg, setActiveImg] = useState(0);
  const [tab, setTab] = useState<"description" | "details" | "shipping" | "reviews">("description");
  const [cartAction, setCartAction] = useState<{ productId: string; status: "idle" | "loading" | "ready" }>({ productId: product.id, status: "idle" });
  const cartButtonTimer = useRef<number | null>(null);
  const cartActionStatus = cartAction.productId === product.id ? cartAction.status : "idle";
  const add = () => cart.add({ id: product.id, name: cleanProductName(product.name), price: product.price, image: img });
  useEffect(() => () => { if (cartButtonTimer.current !== null) window.clearTimeout(cartButtonTimer.current); }, []);
  useEffect(() => { if (window.location.hash === "#reviews") setTab("reviews"); }, []);
  const handleCartAction = () => {
    if (cartActionStatus === "ready") { nav({ to: "/checkout" }); return; }
    ...
    cartButtonTimer.current = window.setTimeout(() => { setCartAction({ productId: product.id, status: "ready" }); cartButtonTimer.current = null; }, 5000);
  };
  const handleBuyNow = () => {
    if (cartActionStatus === "idle") add();
    if (cartButtonTimer.current !== null) { window.clearTimeout(cartButtonTimer.current); cartButtonTimer.current = null; }
    nav({ to: "/checkout" });
  };
  const handleShare = async () => {
    ...
    const shareData = { title: `${cleanProductName(product.name)} — YOMORA`, text: `${cleanProductName(product.name)} · ${formatINR(product.price)} · Handcrafted 925 sterling silver jewellery from YOMORA.`, url: productUrl };
    if (navigator.share) {
      try { await navigator.share(shareData); return; } catch (error) { if (error instanceof DOMException && error.name === "AbortError") return; }
    }
    try { await navigator.clipboard.writeText(shareData.url); toast.success("Product link copied"); }
    catch {
  ...
  const toggleWish = () => wishlist.toggle({ id: product.id, name: cleanProductName(product.name), price: product.price, image: img, category: product.category });
  return (
    <div className="min-h-screen bg-background">
      <SiteHeader />
      ...
      <section className="container-x mx-auto max-w-[1400px] py-10">
        <div className={`grid gap-8 lg:gap-8 ${gallery.length > 1 ? "lg:grid-cols-[90px_1fr_1fr]" : "lg:grid-cols-2"}`}>
          <div className={`order-2 lg:order-1 lg:flex-col ${gallery.length > 1 ? "flex" : "hidden"}`}>
            <button className="hidden h-8 w-full items-center justify-center text-muted-foreground hover:text-gold lg:flex"><ChevronUp className="h-4 w-4" /></button>
            <div className="flex gap-3 overflow-x-auto lg:flex-col lg:gap-3 lg:overflow-visible">
              {gallery.map((src, i) => (
                <button key={i} onClick={() => setActiveImg(i)} className={`h-20 w-20 flex-shrink-0 overflow-hidden border-2 bg-secondary/40 transition-colors ${activeImg === i ? "border-gold" : "border-transparent hover:border-gold/40"}`}>
                  <img src={src} alt="" width={160} height={160} loading="lazy" decoding="async" className="h-full w-full object-cover" />
                </button>
              ))}
            </div>
            <button className="hidden h-8 w-full items-center justify-center text-muted-foreground hover:text-gold lg:flex"><ChevronDown className="h-4 w-4" /></button>
          </div>
          <div className="relative order-1 mx-auto w-full max-w-[450px] self-start bg-onyx md:max-w-none lg:order-2">
            <img src={gallery[activeImg]} alt={cleanProductName(product.name)} width={900} height={900} fetchPriority="high" decoding="async" className={`block aspect-square w-full object-cover ${product.sold_out ? "opacity-70" : ""}`} />
            {product.sold_out && (<span className="absolute left-4 top-4 bg-onyx/90 px-3 py-1.5 text-[10px] font-bold tracking-[0.24em] text-cream">SOLD OUT</span>)}
            <div className="absolute right-4 top-4 flex items-center gap-2">
              <button type="button" onClick={toggleWish} aria-label={wished ? "Remove from wishlist" : "Add to wishlist"} className="grid h-10 w-10 place-items-center rounded-full bg-background/90 text-onyx hover:bg-gold"><Heart className={`h-4 w-4 ${wished ? "fill-current text-gold" : ""}`} /></button>
              <button type="button" onClick={handleShare} aria-label="Share this product" className="grid h-10 w-10 place-items-center rounded-full bg-background/90 text-onyx hover:bg-gold"><Share2 className="h-4 w-4" /></button>
            </div>
          </div>
          <div className="order-3 min-w-0">
            <div className="flex flex-wrap gap-2">
              {isProductNew(product) && (<span className="bg-gold px-3 py-1 text-[10px] font-bold tracking-[0.24em] text-onyx">NEW</span>)}
              {product.sold_out && (<span className="bg-onyx px-3 py-1 text-[10px] font-bold tracking-[0.24em] text-cream">SOLD OUT</span>)}
            ...
            <p className="mt-3 text-xs text-muted-foreground">Verified reviews appear below after delivery.</p>
            ...
            {product.sold_out ? (
              <>
                <p className="mt-5 text-sm text-muted-foreground">This piece is currently sold out. Leave your details and we'll alert you as soon as it's restocked.</p>
                <NotifyMeForm productId={product.id} />
              </>
            ) : (
              <div className="mt-5 space-y-3">
                <button type="button" onClick={handleCartAction} disabled={cartActionStatus === "loading"} aria-busy={cartActionStatus === "loading"} className={`relative flex w-full items-center justify-center gap-3 overflow-hidden border px-6 py-4 text-[11px] font-bold tracking-[0.24em] transition-all duration-300 ${cartActionStatus === "ready" ? "border-onyx bg-onyx text-cream hover:border-gold hover:bg-onyx/90" : "border-gold bg-gold text-onyx hover:bg-gold/90"} ${cartActionStatus === "loading" ? "cursor-wait" : "cursor-pointer"}`}>
                  {cartActionStatus === "loading" ? (<><LoaderCircle className="h-4 w-4 animate-spin" /><span>ADDING TO CART</span></>) : cartActionStatus === "ready" ? (<><span>PROCEED TO CHECKOUT</span><ArrowRight className="h-4 w-4" /></>) : (<span>ADD TO CART</span>)}
                </button>
                <button type="button" onClick={handleBuyNow} className="w-full border border-onyx px-6 py-4 text-[11px] font-bold tracking-[0.28em] text-onyx hover:bg-onyx hover:text-cream">BUY NOW</button>
              </div>
            )}
            <div className="mt-4 flex items-center justify-center gap-2 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4 text-gold" /> 100% Secure Payment</div>
          </div>
        </div>
      </section>
      <section className="border-y border-border bg-secondary/30">
        <div className="container-x mx-auto grid max-w-[1400px] grid-cols-2 gap-6 py-6 sm:grid-cols-3 lg:grid-cols-5">
          {[{ icon: Truck, t: "FREE SHIPPING", s: "On All Orders" }, { icon: RotateCcw, t: "EASY RETURNS", s: "Within 7 Days" }, { icon: ShieldCheck, t: "SECURE PAYMENT", s: "100% Safe & Secure" }, { icon: Gift, t: "PREMIUM PACKAGING", s: "Perfect for Gifting" }, { icon: Award, t: "30+ YEARS OF TRUST", s: "Since 1994" }].map(({ icon: Icon, t, s }) => (
            <div key={t} className="flex items-center gap-3"><Icon className="h-8 w-8 text-gold" /><div><p className="text-[11px] font-bold tracking-[0.2em] text-foreground">{t}</p><p className="text-xs text-muted-foreground">{s}</p></div></div>
          ))}
        </div>
      </section>
          ...
          <div>
            <div className="flex flex-wrap gap-6 border-b border-border">
              {(["description", "details", "shipping", "reviews"] as const).map((k) => (
                <button key={k} onClick={() => setTab(k)} className={`-mb-px border-b-2 pb-3 text-[11px] font-bold uppercase tracking-[0.24em] transition-colors ${tab === k ? "border-gold text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
      ...
      {relatedItems.length > 0 && (
        <section className="container-x mx-auto max-w-[1400px] pb-20">
          ...
          <div className="mt-8 grid grid-cols-1 gap-5 min-[400px]:grid-cols-2 min-[400px]:gap-3 sm:gap-6 md:grid-cols-4 xl:grid-cols-5">
            {relatedItems.map((p) => (
              <Link key={p.id} to="/products/$category" params={{ category: p.id }} className="group grid grid-cols-[42%_1fr] items-center gap-2 border border-border/70 bg-secondary/20 p-2 min-[400px]:block min-[400px]:border-0 min-[400px]:bg-background min-[400px]:p-0">
                <div className="relative overflow-hidden bg-secondary/40">
